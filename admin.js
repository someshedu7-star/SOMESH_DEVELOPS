...
function setLoggedInState(username) {
  ...
  adminDescription.textContent = useLocalMode
    ? 'Render backend is unavailable, so admin data is stored only in this browser.'
...
async function loadAdminReviews() {
  ...
  try {
    ...
    useLocalMode = true;
    loadLocalAdminReviews();
    adminStatus.textContent = 'Render backend is unavailable. Admin data works only in this browser temporarily.';
  }
}
...
async function checkAdminStatus() {
  if (useLocalMode) {
    ...
    if (admin && username === admin.username) {
      adminStatus.textContent = 'Render backend is unavailable. Changes stay only in this browser.';
      loadLocalAdminReviews();
      return;
    }
    renderAdminReviews([], false);
    if (admin) setLoginState(); else setSignupState();
    adminStatus.textContent = 'Render backend is unavailable. Local browser mode is active.';
    return;
  }
...
signupForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (!getStoredAdmin()) {
      ...
      setSession(username);
      signupForm.reset();
      adminStatus.textContent = 'Render backend unavailable. Admin account created in this browser instead.';
      loadLocalAdminReviews();
    } else {
      adminStatus.textContent = 'Render backend unavailable and local admin already exists. Please log in.';
      setLoginState();
    }
  }
});
...
loginForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    const admin = getStoredAdmin();
    if (!admin || admin.username !== username || admin.password !== password) {
      adminStatus.textContent = 'Render backend unavailable and local admin login did not match.';
      return;
    }
    setSession(username);
    loginForm.reset();
    adminStatus.textContent = 'Render backend unavailable. Logged in using this browser storage.';
    loadLocalAdminReviews();
  }
});
...
logoutButton.addEventListener('click', async () => {
  ...
  try {
    ...
    useLocalMode = true;
    clearSession();
    adminStatus.textContent = 'Render backend unavailable. Logged out of this browser session.';
    checkAdminStatus();
  }
});
...
adminReviewList.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (review) {
      review.reply = { text: reply, createdAt: new Date().toISOString() };
      saveStoredReviews(reviews);
      adminStatus.textContent = 'Render backend unavailable. Reply saved in this browser instead.';
      loadLocalAdminReviews();
    } else {
      adminStatus.textContent = 'Review not found.';
    }
...
adminReviewList.addEventListener('click', async event => {
  ...
  try {
    ...
    const nextReviews = getStoredReviews().filter(item => item.id !== reviewId);
    saveStoredReviews(nextReviews);
    adminStatus.textContent = 'Render backend unavailable. Review deleted from this browser instead.';
    loadLocalAdminReviews();
  }
});
